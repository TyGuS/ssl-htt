...
| sll_2 of ~~ ((x) == (null)) of
  ...
  exists h_sll_nxtlen1lo1hi1_541,
  (0) <= (len1) /\ (0) <= (v) /\ (hi) == ((if (hi1) <= (v) then v else hi1)) /\ (len) == ((1) + (len1)) /\ (lo) == ((if (v) <= (lo1) then v else lo1)) /\ (v) <= (7) /\ h = x :-> (v) \+ x .+ 1 :-> (nxt) \+ h_sll_nxtlen1lo1hi1_541 /\ sll nxt len1 lo1 hi1 h_sll_nxtlen1lo1hi1_541.
