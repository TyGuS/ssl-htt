...
| bst_2 of (x == null) = false of
  ...
  exists h_bst_lsz1lo1hi1_570 h_bst_rsz2lo2hi2_571,
  0 <= sz1 /\ 0 <= sz2 /\ 0 <= v /\ hi == (if hi2 <= v then v else hi2) /\ hi1 <= v /\ lo == (if v <= lo1 then v else lo1) /\ sz == 1 + sz1 + sz2 /\ v <= 7 /\ v <= lo2 /\ h = x :-> v \+ x .+ 1 :-> l \+ x .+ 2 :-> r \+ h_bst_lsz1lo1hi1_570 \+ h_bst_rsz2lo2hi2_571 /\ bst l sz1 lo1 hi1 h_bst_lsz1lo1hi1_570 /\ bst r sz2 lo2 hi2 h_bst_rsz2lo2hi2_571.

Lemma pure75 : 0 == 0. Admitted.
Hint Resolve pure75: ssl_pure.
Lemma pure76 : 7 == 7. Admitted.
Hint Resolve pure76: ssl_pure.
Lemma pure77 hi2x lo2x hi1x vx2 lo1x : hi1x <= vx2 -> vx2 <= lo2x -> (if hi2x <= vx2 then vx2 else hi2x) <= 7 -> 0 <= vx2 -> vx2 <= 7 -> 0 <= (if vx2 <= lo1x then vx2 else lo1x) -> 0 <= lo1x. Admitted.
Hint Resolve pure77: ssl_pure.
Lemma pure78 hi2x lo2x hi1x vx2 lo1x : hi1x <= vx2 -> vx2 <= lo2x -> (if hi2x <= vx2 then vx2 else hi2x) <= 7 -> 0 <= vx2 -> vx2 <= 7 -> 0 <= (if vx2 <= lo1x then vx2 else lo1x) -> hi1x <= 7. Admitted.
Hint Resolve pure78: ssl_pure.
Lemma pure79 sz1x sz2x : 0 <= sz2x -> 0 <= sz1x -> 0 <= 1 + sz1x + sz2x -> 1 + sz1x + sz2x == 1 + sz1x + sz2x. Admitted.
Hint Resolve pure79: ssl_pure.
Lemma pure80 hi2x lo2x hi1x vx2 lo1x2 : 0 <= (if vx2 <= lo1x2 then vx2 else lo1x2) -> hi1x <= vx2 -> vx2 <= lo2x -> (if hi2x <= vx2 then vx2 else hi2x) <= 7 -> 0 <= vx2 -> vx2 <= 7 -> (if hi2x <= vx2 then vx2 else hi2x) == (if hi2x <= vx2 then vx2 else hi2x). Admitted.
Hint Resolve pure80: ssl_pure.
Lemma pure81 hi2x lo2x hi1x vx2 lo1x2 : 0 <= (if vx2 <= lo1x2 then vx2 else lo1x2) -> hi1x <= vx2 -> vx2 <= lo2x -> (if hi2x <= vx2 then vx2 else hi2x) <= 7 -> 0 <= vx2 -> vx2 <= 7 -> (if vx2 <= lo1x2 then vx2 else lo1x2) == (if vx2 <= lo1x2 then vx2 else lo1x2). Admitted.
...
ex_elim h_bst_lx2sz1xlo1x2hi1x_570x h_bst_rx2sz2xlo2xhi2x_571x.
...
move=>[sigma_bst_xszlohi_a].
subst h_bst_xszlohi_a.
move=>[H_bst_lx2sz1xlo1x2hi1x_570x H_bst_rx2sz2xlo2xhi2x_571x].
ssl_read x.
ssl_read (x .+ 1).
ssl_call_pre (retv :-> unused2 \+ h_bst_lx2sz1xlo1x2hi1x_570x).
ssl_call (lo1x2, sz1x, hi1x, unused2).
exists (h_bst_lx2sz1xlo1x2hi1x_570x);
sslauto.
move=>h_call1.
...
ssl_write_post retv.
ssl_emp;
exists (x :-> vx2 \+ x .+ 1 :-> lx2 \+ x .+ 2 :-> rx2 \+ h_bst_lx2sz1xlo1x2hi1x_c1 \+ h_bst_rx2sz2xlo2xhi2x_571x);
sslauto;
solve [
unfold_constructor 2;
exists (sz1x), (sz2x), (vx2), (hi2x), (hi1x), (lo1x2), (lo2x), (lx2), (rx2);
exists (h_bst_lx2sz1xlo1x2hi1x_c1);
exists (h_bst_rx2sz2xlo2xhi2x_571x);
sslauto ].
Qed.
