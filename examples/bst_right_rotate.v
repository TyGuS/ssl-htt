...
| bst_2 of (x == null) = false of
  ...
  exists h_bst_lsz1lo1hi1_574 h_bst_rsz2lo2hi2_575,
  0 <= sz1 /\ 0 <= sz2 /\ 0 <= v /\ hi == (if hi2 <= v then v else hi2) /\ hi1 <= v /\ lo == (if v <= lo1 then v else lo1) /\ sz == 1 + sz1 + sz2 /\ v <= 7 /\ v <= lo2 /\ h = x :-> v \+ x .+ 1 :-> l \+ x .+ 2 :-> r \+ h_bst_lsz1lo1hi1_574 \+ h_bst_rsz2lo2hi2_575 /\ bst l sz1 lo1 hi1 h_bst_lsz1lo1hi1_574 /\ bst r sz2 lo2 hi2 h_bst_rsz2lo2hi2_575.

Lemma pure88 sz1l2 sz2l2 sz2 : 0 <= sz2l2 -> 0 <= sz2 -> 0 <= 1 + sz1l2 + sz2l2 -> 0 <= sz1l2 -> sz1l2 + 1 + sz2l2 + sz2 == 1 + sz1l2 + sz2l2 + sz2. Admitted.
Hint Resolve pure88: ssl_pure.
Lemma pure89 sz2l2 sz2 sz1l2 : 0 <= sz2l2 -> 0 <= sz2 -> 0 <= 1 + sz1l2 + sz2l2 -> 0 <= sz1l2 -> 0 <= 1 + sz2l2 + sz2. Admitted.
Hint Resolve pure89: ssl_pure.
Lemma pure90 lo2l2 v2 vl22 hi2l2 lo2 hi1l2 : (if hi2l2 <= vl22 then vl22 else hi2l2) <= v2 -> hi1l2 <= vl22 -> vl22 <= 7 -> 0 <= v2 -> v2 <= lo2 -> 0 <= vl22 -> vl22 <= lo2l2 -> v2 <= 7 -> hi2l2 <= v2. Admitted.
Hint Resolve pure90: ssl_pure.
Lemma pure91 lo2l2 v2 vl22 hi2l2 lo2 hi1l2 : (if hi2l2 <= vl22 then vl22 else hi2l2) <= v2 -> hi1l2 <= vl22 -> vl22 <= 7 -> 0 <= v2 -> v2 <= lo2 -> 0 <= vl22 -> vl22 <= lo2l2 -> v2 <= 7 -> vl22 <= (if v2 <= lo2l2 then v2 else lo2l2). Admitted.
...
Definition bst_right_rotate_type :=
  ...
  STsep (
    ...
    [vfun (_: unit) h =>
      ...
      exists h_bst_l3sz3lo3hi3_576 h_bst_xsz4lo4hi4_577,
      0 <= sz3 /\ 0 <= sz4 /\ 0 <= v3 /\ hi3 <= v3 /\ sz3 + sz4 == sz1 + sz2 /\ v3 <= 7 /\ v3 <= lo4 /\ h = retv :-> y \+ y :-> v3 \+ y .+ 1 :-> l3 \+ y .+ 2 :-> x \+ h_bst_l3sz3lo3hi3_576 \+ h_bst_xsz4lo4hi4_577 /\ bst l3 sz3 lo3 hi3 h_bst_l3sz3lo3hi3_576 /\ bst x sz4 lo4 hi4 h_bst_xsz4lo4hi4_577
    ]).
...
ex_elim h_bst_ll22sz1l2lo1l2hi1l2_574l2 h_bst_rl22sz2l2lo2l2hi2l2_575l2.
...
move=>[H_bst_ll22sz1l2lo1l2hi1l2_574l2 H_bst_rl22sz2l2lo2l2hi2l2_575l2].
ssl_read (l2 .+ 2).
ssl_write (l2 .+ 2).
ssl_write_post (l2 .+ 2).
ssl_write retv.
...
exists (h_bst_ll22sz1l2lo1l2hi1l2_574l2);
exists (x :-> v2 \+ x .+ 1 :-> rl22 \+ x .+ 2 :-> r2 \+ h_bst_rl22sz2l2lo2l2hi2l2_575l2 \+ h_bst_r2sz2lo2hi2_b);
sslauto;
solve [
unfold_constructor 2;
exists (sz2l2), (sz2), (v2), (hi2), (hi2l2), (lo2l2), (lo2), (rl22), (r2);
exists (h_bst_rl22sz2l2lo2l2hi2l2_575l2);
