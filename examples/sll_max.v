...
Lemma pure4 hi1x2 vx2 : vx2 <= 7 -> 0 <= vx2 -> (if hi1x2 <= vx2 then vx2 else hi1x2) == (if hi1x2 <= vx2 then vx2 else hi1x2). Admitted.
...
Definition sll_max_type :=
  ...
  STsep (
    ...
    [vfun (_: unit) h =>
      ...
      h = r :-> hi \+ h_sll_xnlohi_515 /\ sll x n lo hi h_sll_xnlohi_515
    ]).
...
sslauto ].
ex_elim len1x vx2 hi1x2 lo1x nxtx2.
ex_elim h_sll_nxtx2len1xlo1xhi1x2_513x.
...
move=>[sigma_sll_xnlohi_514].
subst h_sll_xnlohi_514.
move=>H_sll_nxtx2len1xlo1xhi1x2_513x.
ssl_read x.
ssl_read (x .+ 1).
ssl_call_pre (r :-> a2 \+ h_sll_nxtx2len1xlo1xhi1x2_513x).
ssl_call (len1x, lo1x, hi1x2, a2).
exists (h_sll_nxtx2len1xlo1xhi1x2_513x);
sslauto.
move=>h_call1.
ex_elim h_sll_nxtx2len1xlo1xhi1x2_5151.
move=>[sigma_call1].
subst h_call1.
move=>H_sll_nxtx2len1xlo1xhi1x2_5151.
store_valid.
ssl_read r.
ssl_write r.
ssl_write_post r.
ssl_emp;
exists (x :-> vx2 \+ x .+ 1 :-> nxtx2 \+ h_sll_nxtx2len1xlo1xhi1x2_5151);
sslauto;
solve [
unfold_constructor 2;
exists (len1x), (vx2), (hi1x2), (lo1x), (nxtx2);
exists (h_sll_nxtx2len1xlo1xhi1x2_5151);
sslauto ].
Qed.
